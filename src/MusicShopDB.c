...
const char* ownerPassword = "1111";
int accessRights = CLIENT_RIGHTS;
...
void Authorization(void) {
    ...
    if (strcmp(name, ownerLogin) == 0) {
        printf("Enter your password:\n");
        char pass[10];
        scanf("%9s", pass);
        
        int attempts = 0;
        while (strcmp(pass, ownerPassword) != 0 && attempts < ATTEMPTS_AMOUNT) {
            printf("Incorrect password, please try again:\n");
            scanf("%9s", pass);
            attempts++;
        }
        
        if (strcmp(pass, ownerPassword) == 0) {
...
void GetPeriodTrade(sqlite3* db, char* date1, char* date2) {
    char sql[512];  // Объявляем массив символов
...
int Insert(sqlite3 *db, char *zErrMsg, int ret){
    ...
    while (go) {
        ...
        for(int i = 0; values[i] != '\0' && values[i] != '\n'; i++) {
            ...
            if(pos == 0) date[i] = values[i];
            if(pos == 1) compactID = compactID * 10 + (values[i] - '0');
            if(pos == 2) amount = amount * 10 + (values[i] - '0');
            if(pos == 3) code = code * 10 + (values[i] - '0');
        }
                
        GetLastDate(db);
        for(int i = 1; i < 11; i++) date[i-1] = date[i];
        date[10] = '\0';
...
char* GetLastDate(sqlite3* db){
	...
	char* zErrMsg = 0;
    (void)sqlite3_exec(db, sql, callback3, (void*)NULL, &zErrMsg);
	return LastDate;
}
